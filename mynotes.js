$(function(){
	...
	$.ajax({
		...
		error: function(){
			$('#alertContent').text("There was an error with the Ajax call. Plese try again later.!");
			$('#alert').fadeIn();
		}
	});
...
$('#addNote').click(function(){
	$.ajax({
		...
		success: function(data){
			if (data == 'error') {
				...
				$("textarea").val("");
				//show hide elements
				showHide(["#notePad","#allNotes"], ["allNotes","#notes","#addNote","#edit","#done"]);
				$("textarea").focus();
			}
	...
	$("textarea").keyup(function(){
		...
		url:"updatenote.php",
		type:"POST",
		//we need to send the current note content with its is to the php file
